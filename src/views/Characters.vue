  ...
  <div id="app">
    ...
    <div v-if="selectedCharacter" class="character-form">
      ...
      <form @submit.prevent="submitCharacter">
        ...
        <button @click="addPrompt">添加新会话</button>
    ...
    是否发布
		  ...
		  <div>
		      <div>
		        <input type="radio" value="1" v-model="selectedCharacter.publish" />
		        是
		      </div>
		      <div>
		        <input type="radio" value="0" v-model="selectedCharacter.publish" />
		        否
		      </div>
		    </div>
...
export default {
  ...
  methods: {
      ...
      this.characters = [
        {
          ...
          character_name: "角色1",
          summery: "这是角色1的简介",
          prompts_texts: [{"role":"system","text":"你是一个....模型"},{"role":"user","text":"你好"},{"role":"assestence","text":"我好"}],
          avatar: "", // base64 图片
    ...
    selectCharacter(character) {
	  ...
	  this.axios.get(`/api/v1/character/`+this.selectedCharacter.id+`?avatar=True`)
	  	.then((result) => {
	  		this.selectedCharacter.avatar = "data:image/png;base64," + result.data.data
			
	  	})
	  	.catch((err) => {});
		
	  this.axios.get(`/api/v1/character/`+this.selectedCharacter.id+`?audio=True`)
		.then((result) => {
			this.selectedCharacter.audio_data = "data:audio/wav;base64,"+ result.data.data
